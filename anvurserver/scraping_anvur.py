...
headers = {"User-Agent": "Anvur Journal Acquisition", "Mail": "luigi.palumbo@unitus.it"}
...
link_list = [
    ...
    for item in response.find_all("a", {"href": re.compile("pdf$")})
]
...
for item in link_list:
    ...
    with requests.get(item.get("link"), headers=headers) as res:
        with open(os.path.join(os.sep, "app", "data", item.get("filename")), "wb") as f:
            f.write(res.content)
...
docs_classe_a = [
    item
    for item in current_anvur_df.to_dict(orient="records")
...
for item in docs_classe_a:
    table_list = camelot.read_pdf(
        os.path.join(os.sep, "app", "data", item.get("filename")), pages="1-end"
    )
...
classe_a_df = reduce(
    lambda left, right: pd.merge(left, right, on=["ISSN"], how="outer"), classe_a_list
)
...
# Combine all titles and ISSN
scientific_journals = pd.concat(
    [df[["TITOLO", "ISSN"]].copy() for df in scientific_list]
)
...
scientific_df = reduce(
    lambda left, right: pd.merge(left, right, on=["ISSN"], how="outer"), scientific_list
)
...
anvur_journals = (
    pd.concat([classe_a_journals, scientific_journals])
    .drop_duplicates(subset="ISSN")
    .sort_values(by="TITOLO")
)


# Save data
db = TinyDB(os.path.join(os.sep, "app", "db.json"))
