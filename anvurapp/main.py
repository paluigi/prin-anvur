...
import flet as ft
from update_data import get_scimago_data, get_anvur_data
...
def main(page):
    ...
    def details_click(e, issn, title):
        ...
        if len(scientific_details) == 1:
            scientific_column = []
            scientific_column.append(
                ft.Text("Scientific sectors", size=20, weight=ft.FontWeight.BOLD)
            )
            # Get dict inside list and pop ISSN
            scientific_listview = ft.ListView(expand=True, spacing=5)
            scientific_details = {
                k: v for k, v in scientific_details[0].items() if k != "ISSN"
            }
        ...
        if len(classea_details) == 1:
            classea_column = []
            classea_column.append(
                ft.Text("Classe A", size=20, weight=ft.FontWeight.BOLD)
            )
            # Get dict inside list and pop ISSN
            classea_listview = ft.ListView(expand=True, spacing=5)
            classea_details = {
                k: v for k, v in classea_details[0].items() if k != "ISSN"
            }
        ...
        if len(scimago_details) == 1:
            scimago_column = []
            scimago_column.append(
                ft.Text("Scimago JR", size=20, weight=ft.FontWeight.BOLD)
            )
            # link in Button
            scimago_button = ft.ElevatedButton(
                "View on Scimago JR", url=scimago_details[0].get("Link")
            )
            # Get dict inside list and pop ISSN
            scimago_listview = ft.ListView(expand=True, spacing=5)
            scimago_details = {
                k: v for k, v in scimago_details[0].items() if k in scimago_fields
            }
        ...
        details.controls.append(
            ft.ResponsiveRow([ft.Column(col=4, controls=res) for res in search_results])
        )
        details.controls.append(
            ft.Text(
                "Notes: ‡(year) means included until 31.12.year; S/A(year) means included from 01.01.year",
                size=10,
            )
        )
        details.controls.append(
            ft.ElevatedButton("Clear details", on_click=details_clear)
        )
        page.update()
    ...
    def search_click(e):
        if not txt_search.value:
            ...
            results = anvur.search(
                Journal.TITOLO.search(f"{fragment}+", flags=re.IGNORECASE)
            )
            ...
            for res in results:
                lv.controls.append(
                    ft.ResponsiveRow(
                        [
                            ft.Column(
                                col={"sm": 5}, controls=[ft.Text(res.get("TITOLO"))]
                            ),
                            ft.Column(
                                col={"sm": 4},
                                controls=[ft.Text("ISSN: {}".format(res.get("ISSN")))],
                            ),
                            ft.Column(
                                col={"sm": 3},
                                controls=[
    ...
    def update_scimago(e):
        ...
        scimago.clear_cache()
        alert = ft.AlertDialog(title=ft.Text(result))
        page.open(alert)
        ...
        ft.ResponsiveRow(
            [
                ft.Column(
                    ...
                    controls=[
                        ft.Text(
                            "ANVUR Journal search", size=50, weight=ft.FontWeight.BOLD
                        )
                    ],
                ),
                ft.Column(
                    col={"sm": 3},
                    controls=[ft.ElevatedButton("Update Anvur", on_click=update_anvur)],
                ),
                ft.Column(
                    col={"sm": 3},
                    controls=[
                        ft.ElevatedButton("Update Scimago JR", on_click=update_scimago)
                    ],
                ),
            ]
        ),
